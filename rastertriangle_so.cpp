...
/*
void calculate_triangle_area(int index, float* triangle_area, float* points_onface, int m1, int m2, int m3){
    x1 = points_onface[m1*2+0];
    y1 = points_onface[m1*2+1];
    x2 = points_onface[m2*2+0];
    y2 = points_onface[m2*2+1];
    x3 = points_onface[m3*2+0];
    y3 = points_onface[m3*2+1];

    a = sqrt((y2-y1)**2 + （x2-x1)**2);
    b = sqrt((y3-y1)**2 + （x3-x1)**2);
    c = sqrt((y3-y2)**2 + （x3-x2)**2);
    s=0.5*(a+b+c);
    area=sqrt(s*(s-a)*(s-b)*(s-c));
    triangle_area[index] = area;
}
*/
...
void cover_rgbd(float* p , float* pz, int m1,int m2,int m3, int idx, int* triangle ,float* zbuf, int h,int w, int * xys, int* triangle_area){
...
void rgbzbuffer(int h,int w, float* points_onface, float* points_onface_ori, float* points_z, int len_mesh, int* mesh, float* zbuf, int* triangle,int* triangle_area){
	...
	for (int i=0; i<len_mesh; i++){
		cover_rgbd(
			...
			mesh[i*3+1], mesh[i*3+2], i, triangle, zbuf,h, w, xys, triangle_area);
	}
